...
#define WRAPPED_MEMBER(name)                                                                                           \
    MemberWrapper<decltype(&IcepickMenu::##name), &IcepickMenu::##name, decltype(&Menu), &Menu>::Call
                         ...
                         FileSystemManager& fsManager)
{
    ...
    // Commands
    conCommandManager.RegisterCommand("show_icepick_menu", WRAPPED_MEMBER(ShowMenuCommand), "Shows the Icepick Menu",
                                      0);
    conCommandManager.RegisterCommand("hide_icepick_menu", WRAPPED_MEMBER(HideMenuCommand), "Hides the Icepick Menu",
                                      0);

    conCommandManager.RegisterCommand("toggle_toolgun", WRAPPED_MEMBER(ToggleToolgunCommand), "Toggle the toolgun", 0);
    conCommandManager.RegisterCommand("undo", WRAPPED_MEMBER(DummyConCommand),
                                      "Dummy function for scripted undo functionality", 0);
    conCommandManager.RegisterCommand("instant_respawn", WRAPPED_MEMBER(DummyConCommand),
                                      "Dummy function for scripted instant respawn", 0);

    sqManager.AddFuncRegistration(CONTEXT_CLIENT, "int", "IsSpawnMenuOpen", "", "Help text",
                                  WRAPPED_MEMBER(IsMenuShowing));
    sqManager.AddFuncRegistration(CONTEXT_SERVER, "int", "IsInvincibilityEnabled", "", "Help text",
                                  WRAPPED_MEMBER(IsInvincibilityEnabled));

    sqManager.AddFuncRegistration(CONTEXT_SERVER, "void", "EnableEditMode", "", "Help text",
                                  WRAPPED_MEMBER(EnableEditMode));
    sqManager.AddFuncRegistration(CONTEXT_SERVER, "void", "DisableEditMode", "", "Help text",
                                  WRAPPED_MEMBER(DisableEditMode));
    sqManager.AddFuncRegistration(CONTEXT_SERVER, "int", "IsEditModeEnabled", "", "Help text",
                                  WRAPPED_MEMBER(IsEditModeEnabled));
    sqManager.AddFuncRegistration(CONTEXT_CLIENT, "int", "IsEditModeEnabled", "", "Help text",
                                  WRAPPED_MEMBER(IsEditModeEnabled));

    sqManager.AddFuncRegistration(CONTEXT_CLIENT, "void", "ClearTools", "", "Help text", WRAPPED_MEMBER(ClearTools));
    sqManager.AddFuncRegistration(CONTEXT_CLIENT, "void", "RegisterTool",
                                  "string id, string friendlyName, string tooltip", "Help text",
                                  WRAPPED_MEMBER(RegisterTool));
    sqManager.AddFuncRegistration(CONTEXT_CLIENT, "void", "AddDividerOption", "string toolId", "Help text",
                                  WRAPPED_MEMBER(AddToolOption_Divider));
    sqManager.AddFuncRegistration(CONTEXT_CLIENT, "void", "AddTextOption", "string toolId, string text", "Help text",
                                  WRAPPED_MEMBER(AddToolOption_Text));
    sqManager.AddFuncRegistration(CONTEXT_CLIENT, "void", "AddButtonOption",
                                  "string toolId, string id, string buttonText", "Help text",
                                  WRAPPED_MEMBER(AddToolOption_Button));
    sqManager.AddFuncRegistration(CONTEXT_CLIENT, "void", "AddSliderOption",
                                  "string toolId, string id, string name, float default, float min, float max",
                                  "Help text", WRAPPED_MEMBER(AddToolOption_Slider));
    sqManager.AddFuncRegistration(CONTEXT_CLIENT, "void", "AddIntSliderOption",
                                  "string toolId, string id, string name, int default, int min, int max", "Help text",
                                  WRAPPED_MEMBER(AddToolOption_IntSlider));

    sqManager.AddFuncRegistration(CONTEXT_SERVER, "void", "ClearSaveBuffer", "", "Help text",
                                  WRAPPED_MEMBER(ClearSaveBuffer));
    sqManager.AddFuncRegistration(CONTEXT_SERVER, "void", "AddSaveItem", "string data", "Help text",
                                  WRAPPED_MEMBER(AddSaveItem));
    sqManager.AddFuncRegistration(CONTEXT_SERVER, "void", "WriteSaveBufferToFile", "string fileName", "Help text",
                                  WRAPPED_MEMBER(WriteSaveBufferToFile));

    sqManager.AddFuncRegistration(CONTEXT_CLIENT, "array", "GetSaveFiles", "", "Help text",
                                  WRAPPED_MEMBER(GetSaveFiles));
    sqManager.AddFuncRegistration(CONTEXT_SERVER, "string", "LoadSaveFileContents", "string fileName", "Help text",
                                  WRAPPED_MEMBER(LoadSaveFileContents));
    sqManager.AddFuncRegistration(CONTEXT_SERVER, "void", "ShowLoadingModal", "", "Help text",
                                  WRAPPED_MEMBER(ShowLoadingModal));
    sqManager.AddFuncRegistration(CONTEXT_SERVER, "void", "HideLoadingModal", "", "Help text",
                                  WRAPPED_MEMBER(HideLoadingModal));
...
SQInteger IcepickMenu::RegisterTool(HSQUIRRELVM v)
{
    ...
    std::sort(m_Tools.begin(), m_Tools.end(), [](Tool& a, Tool& b) { return a.FriendlyName < b.FriendlyName; });
...
void IcepickMenu::DrawOptionsGui()
{
    ...
    for (ToolOption& option : m_ViewingTool->Options)
    {
        ...
        case ToolOptionType::Slider:
            ...
            {
                std::string command =
                    "Spawnmenu_UpdateToolOption( \"" + option.Id + "\", " + std::to_string(option.FloatValue) + " )";
                SDK().GetSQManager().ExecuteClientCode(command.c_str());
                SDK().GetSQManager().ExecuteServerCode(command.c_str());
            }
            break;
        case ToolOptionType::IntSlider:
            ...
            {
                std::string command =
                    "Spawnmenu_UpdateToolOption( \"" + option.Id + "\", " + std::to_string(option.IntValue) + " )";
                SDK().GetSQManager().ExecuteClientCode(command.c_str());
                SDK().GetSQManager().ExecuteServerCode(command.c_str());
            }
            break;
    ...
    ImGui::Begin("Save As", nullptr, ImGuiWindowFlags_NoCollapse);
    {
        ...
        if (ImGui::Button("Save", ImVec2(ImGui::GetWindowContentRegionWidth() * 0.5f - 5, 0)))
        {
            if (IsSaveInputNameValid())
            {
                ...
                SDK().GetSQManager().ExecuteClientCode(
                    ("Spawnmenu_OnSavedGameToFile(\"" + saveFileName + "\");").c_str());
                m_SaveAsOpen = false;
...
void IcepickMenu::DrawCreateSpawnlist()
{
    ...
    ImGui::Begin("Create Spawnlist", nullptr,
...
void IcepickMenu::DrawCallback()
{
    ...
    ImGui::SetNextWindowSize(
        ImVec2(ImGui::GetIO().DisplaySize.x - Padding * 2.0f, ImGui::GetIO().DisplaySize.y - Padding * 2.0f));
    ImGui::SetNextWindowPos(ImVec2(Padding, Padding));
    ImGui::Begin("Icepick", nullptr,
        ...
        if (ImGui::BeginMenuBar())
        {
            ...
            if (ImGui::BeginMenu("Spawnlists"))
            {
                if (ImGui::BeginMenu("Spawnlist Style"))
                {
                    ...
                    if (ImGui::MenuItem("Browser Style", nullptr,
            ...
            if (ImGui::BeginMenu("Options"))
            {
                ...
                if (ImGui::MenuItem("Invincibility", nullptr, m_IsInvincibilityEnabled))
                {
                    ...
                    SDK().GetSQManager().ExecuteServerCode(m_IsInvincibilityEnabled
                                                               ? "Spawnmenu_ChangePlayerInvincibility( true );"
                                                               : "Spawnmenu_ChangePlayerInvincibility( false );");
                }
    ...
    if (m_ShowLoadingModal)
    {
        ...
        ImGui::SetNextWindowPos(ImVec2(io.DisplaySize.x * 0.5f, io.DisplaySize.y * 0.5f), ImGuiCond_Appearing,
                                ImVec2(0.5f, 0.5f));
        ImGui::Begin("Loading save...", nullptr,
                     ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoCollapse);
        {
